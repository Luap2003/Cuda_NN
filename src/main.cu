...
int main() {
  ...
  x_train_host = load_mnist_images("../data/mnist_data/train-images.idx3-ubyte",
                                   &num_samples, &input_size);
  y_train_host = load_mnist_labels("../data/mnist_data/train-labels.idx1-ubyte",
                                   &num_samples, &output_size);

  // For simplicity, allocate random data (replace this with actual data
  // loading)
  ...
  cudaMemcpy(d_x_train, x_train_host, num_samples * input_size * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_y_train, y_train_host, num_samples * output_size * sizeof(float),
             cudaMemcpyHostToDevice);
  ...
  for (int epoch = 0; epoch < epochs; ++epoch) {
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
      ...
      // Backward pass
      backpropagation(network, d_batch_input, d_batch_labels, batch_size,
                      learning_rate);
