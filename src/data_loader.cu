...
float *load_mnist_images(const char *filename, int *number_of_images,
                         int *image_size) {
  ...
  *image_size = num_rows * num_cols;
  ...
  for (int i = 0; i < num_images; i++) {
    size_t read =
        fread(temp_image, sizeof(unsigned char), num_rows * num_cols, fp);
    ...
    for (int j = 0; j < num_rows * num_cols; j++) {
      images[i * num_rows * num_cols + j] =
          temp_image[j] / 255.0f; // Normalize pixel values to [0,1]
    }
  }
...
float *load_mnist_labels(const char *filename, int *number_of_labels,
                         int *num_classes) {
  FILE *fp = fopen(filename, "rb");
  if (!fp) {
    printf("Could not open %s\n", filename);
