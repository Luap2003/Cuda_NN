 ...
 *
 * Allocates memory for the layer structure, initializes weights using Xavier
 * initialization, sets biases to zero, and transfers weights and biases to the
 * GPU.
 *
 ...
 *
 * Executes matrix multiplication between input and weights, adds biases, and
...
void backward_layer(Layer *current_layer, Layer *next_layer,
                    float *d_output_delta, float *d_input_grad, int batch_size);
...
void backward_layer(Layer *layer, float *d_input, float *d_output_grad,
                    float *d_input_grad, int batch_size);

void compute_gradients(Layer *layer, float *d_input, float *d_delta,
                       int batch_size);
...
/**
 * @brief Prints the properties and a snippet of the weights and biases of a
 * layer.
 *
